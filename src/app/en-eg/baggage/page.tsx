        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <InfoCard>
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 bg-bg-alt rounded-xl flex items-center justify-center shrink-0 text-xl">🛄</div>
                  <div>
                    <h3 className="font-bold text-text-primary mb-1">{isAr ? "الدرجة الاقتصادية" : "Economy Class"}</h3>
                    <p className="text-sm text-text-secondary mb-2">{isAr ? "حقيبة واحدة — 7 كجم" : "1 bag — 7 kg"}</p>
              ...
              <InfoCard>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-text-secondary mb-2">{isAr ? "حقيبتان — 7 كجم لكل منهما" : "2 bags — 7 kg each"}</p>
          ...
          <section>
            ...
            <InfoCard className="border-red-200 bg-red-50">
              <ul className="space-y-3">
                {[
                  {
                    en: "Liquids over 100 ml in cabin baggage (applies to bottles, cans, tubes, and gels)",
                    ar: "السوائل التي تزيد عن 100 مل في أمتعة المقصورة (تشمل الزجاجات والعلب والأنابيب والمواد الهلامية)",
                    icon: "💧",
                  },
                  {
                    en: "Sharp objects: knives, scissors (blade > 6 cm), razor blades, and needles",
                    ar: "الأغراض الحادة: السكاكين، المقصات (الشفرة أكبر من 6 سم)، شفرات الحلاقة، والإبر",
                    icon: "🔪",
                  },
                  {
                    en: "Flammable materials: lighters with fuel, matches, aerosol sprays",
                    ar: "المواد القابلة للاشتعال: الولاعات المحتوية على وقود، الكبريت، رذاذ الهواء المضغوط",
                    icon: "🔥",
                  },
                  {
                    en: "Explosive or corrosive substances of any kind",
                    ar: "المواد المتفجرة أو الأكّالة من أي نوع",
                    icon: "⚠️",
                  },
                ].map((item, i) => (
                  <li key={i} className={cn("flex gap-3 text-sm text-text-secondary", isAr ? "flex-row-reverse text-right" : "")}>
          ...
          <section>
            ...
            <InfoCard>
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                {[
                  { en: "Sports Equipment", ar: "المعدات الرياضية", detail_en: "Bicycles, golf clubs, surfboards, diving gear", detail_ar: "دراجات، عصي غولف، ألواح تزلج، معدات غوص", icon: "⚽" },
                  { en: "Musical Instruments", ar: "الآلات الموسيقية", detail_en: "Guitars, violins, keyboards (may require extra seat)", detail_ar: "غيتار، كمان، لوحات مفاتيح (قد تستلزم مقعدًا إضافيًا)", icon: "🎸" },
                  { en: "Medical Devices", ar: "الأجهزة الطبية", detail_en: "Oxygen concentrators, wheelchairs, CPAP machines", detail_ar: "أجهزة تركيز الأكسجين، الكراسي المتحركة، أجهزة CPAP", icon: "🏥" },
                ].map((item) => (
                  <div key={item.en} className={cn("p-4 bg-bg-alt rounded-xl text-center", isAr ? "text-right" : "")}>
                    <div className="text-3xl mb-2 text-center">{item.icon}</div>
                    <h4 className="font-bold text-sm text-text-primary mb-1 text-center">{isAr ? item.ar : item.en}</h4>
                    <p className="text-xs text-text-muted text-center">{isAr ? item.detail_ar : item.detail_en}</p>
                  </div>
                ))}
              </div>
            </InfoCard>
          ...
          <section>
            <InfoCard className="bg-brand-red border-brand-red">
              <h2 className="text-lg font-bold text-white mb-4">{isAr ? "نصائح للأمتعة" : "Baggage Tips"}</h2>
              <ul className="space-y-2">
                {[
                  ...
                  { en: "Arrive at the airport at least 2 hours before domestic and 3 hours before international flights.", ar: "احضر إلى المطار قبل ساعتين على الأقل للرحلات الداخلية وثلاث ساعات للدولية." },
                  ...
                  <li key={i} className={cn("flex gap-2 text-sm text-white/90", isAr ? "flex-row-reverse text-right" : "")}>
