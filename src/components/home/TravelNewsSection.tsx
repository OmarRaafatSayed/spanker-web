    ...
    <section className="section-green-dark py-10 sm:py-16" aria-labelledby="travel-news-title">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-5 sm:mb-8">
          ...
          <Link href="/en-eg/travel-news" className="text-xs sm:text-sm font-semibold text-brand-yellow hover:text-brand-yellow/80 flex items-center gap-1">
            {s.viewAll}
          ...
          {s.articles.map((article, i) => (
            ...
            >
              ...
              <div className="p-4 sm:p-5">
                <p className="text-xs text-white/40 mb-1.5 sm:mb-2">{article.date}</p>
                <h3 className="text-sm sm:text-base font-bold text-white mb-2 sm:mb-3 leading-snug group-hover:text-brand-yellow transition-colors line-clamp-2">
                  {article.title}
                </h3>
                <p className="text-xs sm:text-sm text-white/60 leading-relaxed mb-3 sm:mb-4 line-clamp-2 sm:line-clamp-3">
                  {article.excerpt}
                </p>
                <Link
                  href={`/en-eg/${ARTICLE_SLUGS[i]}`}
                  className="inline-flex items-center gap-1 text-xs sm:text-sm font-semibold text-brand-yellow hover:gap-2 transition-all"
                >
                  {s.readMore}
